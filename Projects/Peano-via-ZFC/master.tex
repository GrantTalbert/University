...
\begin{construction}\label{const:pair}
	...
	.\]
	This formula is well-defined, and thus $\left\{ x,y \right\} $ exists by the axiom schema of restricted comprehension. We can similarly define a set
	\[
...
\begin{definition}[Subset]\label{dfn:subset}
	...
	\[
		(y\subseteq x) \iff \forall z(z\in y \implies z\in x)
	.\]
\end{definition}
...
\begin{definition}\label{dfn:7}
	Let $X$ be a set. We say $X$ is an \emph{inductive set} if $\varnothing \in X$ and, for all $x\in X$, we have $s(x)\in X$.
\end{definition}
...
\begin{construction}\label{const:8}
	...
	.\]
	Since being inductive is a well-defined formula ($X$ is inductive if and only if $(\varnothing \in X)\land \forall x(x\in X \implies s(x)\in X)$), this set $I$ exists by the \hyperref[axiom:comprehension]{axiom schema of restricted comprehension}.
\end{construction}
...
\end{lemma}
\begin{proof}
	($\implies $) Let $x\in \cap\mathcal{F} $. Then by \cref{dfn:9} and the \hyperref[axiom:comprehension]{axiom schema of restricted comprehension}, for all $X\in \mathcal{F} $, we have $x\in X$.
...
\end{lemma}
\begin{proof}
	Let $\varphi (x)$ if and only if $\forall X(X\in \mathcal{I} \implies x\in X)$. By the \hyperref[axiom:comprehension]{axiom schema of restricted comprehension} and \cref{dfn:10}, we have $x\in\mathbb{N} $ if and only if $x\in \cup \mathcal{I} $ and $\varphi (x)$.
...
\end{lemma}
\begin{proof}
	Recall \cref{const:pair}. Let $x$ be a set. Since $\mathcal{X} $ also exists, by the \hyperref[axiom:pairing]{axiom of pairing}, there is a set $z$ with $x,\mathcal{X} \in z$. By the \hyperref[axiom:comprehension]{axiom schema of restricted comprehension}, we can deifne a set
	\[
		\left\{ x \right\} \coloneqq \left\{ a\in z \mid a=x \right\} 
	.\]
	By \hyperref[axiom:comprehension]{axiom schema of restricted comprehension}, we have $a\in \left\{ x \right\} $ if and only if $a\in z$ and $a=x$. Note that $a=x$ implies $a\in z$ since $x\in z$, and by definition of equality in first order logic. Thus, we have $a\in \left\{ x \right\} $ if and only if $a=x$. Now recall that $s(x) = x \cup \left\{ x \right\} $. Note that by construction \ref{const:pair}, we have $\left\{ x \right\} \in \left\{ x,\left\{ x \right\}  \right\} $. By \cref{lma:12}, since $x\in \left\{ x \right\} \in \left\{ x,\left\{ x \right\}  \right\} $, we have
	\[
...
\begin{theorem}[Peano Axioms]\label{thm:peano}
	...
	\begin{enumerate}
		...
		\item If $A\subseteq \mathbb{N} $ is such that $\varnothing \in A$ and $n\in A \implies s(n)\in A$, then $A=\mathbb{N} $. In other words, $\mathbb{N} $ has no proper inductive subset.
	\end{enumerate}
\end{theorem}
\begin{proof}
	(1) By \cref{lma:14}, $\mathbb{N} $ is inductive. By \cref{dfn:7}, $\varnothing \in \mathbb{N}$.
	...
	\begin{proof}
		...
		Now suppose that $m\in A$. Let $n\in s(m) = m \cup \left\{ m \right\} $. By \cref{dfn:union-pair} and the \hyperref[axiom:comprehension]{axiom schema of restricted comprehension}, it follows that $n\in m$ or $n\in \left\{ m \right\} $. If $n\in \left\{ m \right\} $, then by an argument given previously, $n=m$. It suffices tho show that $m\subseteq s(m)$. Indeed, if $k\in m$, then $k\in m$ or $k\in \left\{ m \right\} $, hence $k\in m \cup \left\{ m \right\} =s(m)$ by \cref{lma:12}. Thus, $m \subseteq s(m)$ by \cref{dfn:subset}.

		If $n\notin \left\{ m \right\} $, then $n\in m$ by \cref{lma:12}. Since $m\in A$, by the \hyperref[axiom:comprehension]{axiom schema of restricted comprehension}, we have $n\in m$ implies $n\subseteq m$. Thus, $n\subseteq m$. By the above argument that $m\subseteq s(m)$, we have $n\subseteq m \subseteq s(m)$. By \cref{lma:16}, we have $n \subseteq s(m)$. Therefore, $s(m)\in A$, as all cases have been exhausted.
	...
	\begin{lemma}\label{lma:19}
		Let $x$ be a set. Then $x\notin x$.
	\end{lemma}
	\begin{proof}
		Note that $\left\{ x \right\} $ is nonempty, since $x\in \left\{ x \right\} $. By the logic given in \cref{lma:15}, we also see that $a\in \left\{ x \right\} $ if and only if $a=x$. By the \hyperref[regularity]{axiom of regularity}, since $\left\{ x \right\}$ is nonempty, there is $y$ such that $y\in \left\{ x \right\} $, and there is no $z$ with $z\in y$ and $z\in \left\{ x \right\} $. Since $y\in \left\{ x \right\} $ if and only if $y=x$, we have $y=x$. Thus there is no $z$ with $z\in x$ and $z\in \left\{ x \right\} $. If $x\in x$, then we have $x\in x$ and $x\in \left\{ x \right\} $, a contradiction. Therefore, $x\notin x$.
	\end{proof}



	(4) Let $n,m\in \mathbb{N} $ such that $s(n) = s(m)$. Note that $n\in s(n)=s(m)$ implies $n\in s(m) = m \cup \left\{ m \right\} $. If $n\in \left\{ m \right\} $, then $n=m$, and we are done. Suppose for contradiction that $n\in m$. Then we have $m\in s(m) = s(n)$ implies $m\in s(n)=n\cup \left\{ n \right\} $. If $m\in \left\{ n \right\} $, then $m=n$. Thus $n\in m = n$ implies $n\in n$, a contradiction by \cref{lma:19}. Thus we must take $m\in n$. By \cref{lma:18}, we have $m\subseteq n$. We now have $n\subseteq m$ and $m\subseteq n$, and therefore $m=n$. We thus have $n\in m =n$, and thus $n\in n$, a contradiction by \cref{lma:19}.
\end{proof}

Define $0\coloneqq \varnothing $ to recover the presentation of the Peano axioms from class.
