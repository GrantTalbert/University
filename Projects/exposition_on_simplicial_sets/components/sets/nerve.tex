Given a category $\C$, there is an extremely important construction known as the \emph{nerve} of $\C$, which encodes \emph{all} of the information of $\C$ into a simplicial set. This is an extremely important construction for a number of reasons. For example, Boardman and Vogt proved in \cite{BV73} that nerves are quasi-categories, and in particular the nerve is the canonical way of regarding categories as quasi-categories. See \cite{htt} Proposition 1.1.2.2 for a proof. Although exploring this precisely is beyond the scope of this paper, we trust that the reader understands this is an important result, even if they do not quite understand it. With this in mind, we present the definition of the nerve.
...
\end{tikzcd}\]
    ...
    \begin{itemize}
        ...
        \item and the map is $s^0$ on $| \Delta ^2_{(0,0)} | $,
    \end{itemize}
