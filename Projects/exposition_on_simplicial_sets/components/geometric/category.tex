...
\begin{definition}\label{2.2.1}
    The first ordinal number is defined as the empty set $\varnothing$, and is denoted $0$. The $(n+1)$th ordinal number is defined inductively as $n\cup \{n\}$.
\end{definition}
...
Category theory rarely deals with numbers in the conventional sense of the term, so when we say ``consider the finite ordinal $n$'', we are likely using this as a shorthand for the set $\{0,\ldots,n-1\}$. It is also often convenient to havea second notation for finite ordinals: $[n]$ denotes the set $n+1=\{0,\ldots,n\}$. We then write $[-1]$ for $0=\{\}$. In other words, we are denoting a linearly ordered set $\{0,\ldots,n\}$ by its greatest element.
\end{notation}
