...
\begin{proof}
	...
	*\ar[" \mathrm{id} _x ", r] \ar[" \mathrm{id} _{G\left(x \right) }  "', dr]  & \underline{\mathcal{C} } _\mathcal{V} (x,x)\ar[" G_{x\text{,} x}  ", d]  \\
	...
	\[
		F(\ell _\mathcal{V} ^{-1} )\circ \ell _\mathcal{U} =\lambda \circ (\varphi \otimes 1) \implies F(\ell _\mathcal{V} ^{-1} )=\lambda \circ (\varphi \otimes 1)\circ \lambda_\mathcal{U} ^{-1} 
	.\]
	...
	It thus suffices to show $F$ preserves horizontal and vertical composition. First, we show vertical composition. given by $(\beta\circ \alpha)_x=\beta_x \circ_{\underline{\mathcal{D} } _\mathcal{U} } \alpha_x$. Now let
...
\end{tikzcd}\]
	...
	.\]
	for both of these because ntf components come from units, the composition between them is in fact composition in $\underline{\mathcal{D} } _\mathcal{U} $.
\end{proof}
