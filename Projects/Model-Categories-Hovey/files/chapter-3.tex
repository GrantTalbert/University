...
\begin{remark}\label{3.2.4}
	Total left derived functors between general homotopic categories are usually defined as the Kan extension $\Lan_\gamma (\delta F)$, where $\delta : \mathcal{D}  \to \Ho \mathcal{D} $ and $\gamma : \mathcal{C}  \to \Ho \mathcal{C} $ are the localization functors. An immediate thought is then to consider whether or not these definitions are equivalent. Indeed, since left Quillen functors preserve weak equivalences between cofibrant objects, by identifying $\Ho \mathcal{C} $ with $\mathcal{C} _{cf} $ we can likely factor the composite $\delta \circ F$ through $\Ho \mathcal{C} $ by universality, giving the Kan extension for free. The identification step requires elaboration to formalize.
\end{remark}
