...
\begin{remark}\label{1.1.2}
	...
	\begin{enumerate}
		...
		\item If one only considers the action of dom and cod on objects, the information in the three bullet points is not anything more powerful than the condition that $\beta(f)\circ \alpha(f)$ be well-defined. However, when once considers the action on morphisms, we obtain that for a morphism $(h,k): f \to g$ in $\Map \mathcal{C} $,
			...
			.\]
			Here I use a minor abuse of notation and denote by $\alpha(k)$ the arrow $(\operatorname{cod} \circ \alpha)(h,k)$that replaces $k$ in the square diagram, and similarly for $\beta(h)$. We also obtain that $\alpha(h)=h$ and $\beta(k)=k$. Thus the diagrams
			\[\begin{tikzcd}[row sep = 2.5em, column sep = 2.5em]
...
\end{corollary}
\begin{proof}
	Indeed, that the relations coencide follows by \cref{1.2.12} (5), the fact that they are equivalence relations follows from (4), and the invariance under cylinder/path objects is (5) once again.
\end{proof}
\begin{corollary}\label{1.2.14}
...
\begin{remark}\label{1.2.15}
	From our proof to \cref{1.2.12} (3), we saw that if $b$ is cofibrant, than the inclusions $b\to b\amalg b$ are cofibrations, and thus the arrows $i_0,i_1 : b \to b'$ are trivial cofibrations. Dually, the arrows $p_0,p_1 : x' \to x$ are trivial fibrations when $x$ is fibrant.
\end{remark}
...
\end{corollary}
\begin{proof}
	By universality, it suffices to show $\mathcal{C}_{cf}  /\tildesim $ has the same universal property as described in \cref{1.2.4}. We know already that $\delta$ inverts weak equivalences by \cref{1.2.16}. Let $F : \mathcal{C} _{cf}  \to \mathcal{D} $ be a functor which inverts weak equivalences. Let
	\[\begin{tikzcd}[row sep = 2.5em, column sep = 2.5em]
	...
	Note that we did not show $a'$ was cofibrant and fibrant. We need not show this for $a\amalg a$, since we did not compute a functor on an arrow with $a\amalg a$ as domain or codomain. We define $a'$ by a functorial factorization, and by \cref{1.2.12} it does not matter which cylinder object we choose, so this is fine. By \cref{1.2.15} the arrow $i_0 : a \to a'$ is a cofibration, and since $0\to a \to a'$ is a composition of cofibrations, $a'$ is cofibrant. Since $a$ is fibrant, and $a'\to a$ is a fibration, we have that $a'$ is fibrant as well, concluding the proof.
\end{proof}

\begin{theorem}\label{1.2.18}
...
\end{theorem}
\begin{proof}
	(1) follows by \cref{1.2.17} and \cref{1.2.6}. Investigation of the inverses as noted in \cref{1.2.6} gives the natural isomorphisms of (2). The rest of (2) follows from the natural weak equivalences $Q\Rightarrow 1\Rightarrow R$ as noted in \cref{1.1.16}, and invoking \cref{1.2.12} (3,5). Part (3) is subsumed by \cref{1.2.17}. For (4), let $f : a \to b$ be a map in $\mathcal{C} $ which is inverted by $\gamma$. Then $QR(f)$ is an isomorphism in $\mathcal{C} _{cf} /\tildesim$, and thus $QR(f)$ is necessarily a homotopy equivalence. By \cref{1.2.17}, $QR(f)$ is a weak equivalence. Since $Q\Rightarrow 1$ is a natural weak equivalence,
	\[\begin{tikzcd}[row sep = 2.5em, column sep = 2.5em]
