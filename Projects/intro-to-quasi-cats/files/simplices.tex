...
TODO: topologial simplices, faces, degeneracies
...
We will offer some geometric intuition for simplices, because it will be useful later. We can view simplices as geometric objects, and the coface and codegeneracy maps take a very geometric meaning.
% TODO: this
...
\begin{notation}\label{3.7}
	We write $\sSet $ for the category of simplicial sets, which is the functor category $\Fun(\boldsymbol{\Delta} ^{\mathrm{op}} ,\Set )$. There are a variety of other notations for this category, such as $\mathcal{S} \mathrm{imp} \mathcal{S} \mathrm{et} $ or $\mathcal{S} \mathcal{S} \mathrm{et} $. Given a simplicial set $X$, we write $X_n$ for $X$ evaluated at $[n]$. We write $d_i : [n] \to [n-1]$ for the image on the coface $X(d^i)$, and call $d_i$ a \emph{face operator}. Similarly, we write $s_i=X[s^i]: [n] \to [n+1]$ and call $s_i$ a \emph{degeneracy operator}. We typically do not write $X(\alpha )$ for more general morphisms $\alpha $ in $\boldsymbol{\Delta} $, and rather write $\alpha ^*$ and specify that we do not mean precomposition. We call maps $\alpha ^*$ \emph{simplicial operators}.
\end{notation}
