...
\begin{proposition}[\cite{BV73}]\label{5.2}
...
\end{enumerate}
To see if these definitions work in general, we need to introduce a notion of \emph{composition}, a notion of \emph{homotopy}, and show they are compatible in the claimed manner. Just as in 1-categories, we write $f : x \to y$ to denote that $f\in \mathcal{C} _1$ is an edge of a quasi-category with domain $x$ and codomain $y$.
...
Since this definition of composition agrees with the definitions of objects, morphisms, domain, and codomain in the expected way, we are close to finishing (1) and (4), and even (7) since we could identify 1-categories with their nerves to view them as quasi-categories. However, we still have a few things to show before we can be happy. First, we must show that compositions are \emph{unique up to homotopy}. Recall that we defined composites as the 1st face of the filler of a horn, but the definition of a quasi-category \emph{does not require that this filler be unique}. This means two morphisms can have many composites. We need to define a notion of homotopy, and then we need to show all the different composites are indeed homotopic.
...
\end{enumerate}
and we have shown that composites are well-defined up to homotopy. Moreover, this agrees with the nerve construction in ways described above, meaning we may regard 1-categories as quasi-categories via their nerve. This suffices for conditions (1,2,4,7) for a viable model for an $\infty $-category. We also have likely figured out that $n$-simplices are a good model for $n$-morphisms. First note that 1-simplices are 1-morphisms, and 2-simplicies are homotopies, which are our conventional name for 2-morphisms. Also note that 2-simplices often ``witness'' the equality of various 1-morphisms, and similarly 3-simplices often ``witness'' the equality of various 2-simplices. We could likely thus define 3-simplices as 3-morphisms, and define a (convoluded) composition schema for homotopies. It should be clear that these definitions generalizes to $n$-morphisms, albeit they might be extremely difficult to write down. This is why we usually prefer to work with mapping spaces $\mathcal{C} (x,y)$ if we must go beyond 2-morphisms. This shows (4).

We conclude by showing (5,6): we must construct $\infty $-functors and homotopy categories. The former is simple. An $\infty $-functor should be a functor on objects and $n$-morphisms for all $n$, which respects identity and composition. Morphisms of simplicial sets satisfy all of these: given $f : \mathcal{C}  \to \mathcal{D} $ a morphism of simplicial sets,
\begin{itemize}
...
Recall that a homotopy category should ``identify homotopic morphisms''. We don't need to go beyond 2-morphisms, since the higher homotopic data is encoded in the existence of the 2-morphisms themselves. We thus construct the homotopy category $\hh \mathcal{C} $ of a quasi-category $\mathcal{C} $ as follows:
\begin{itemize}
	\item Objects are vertices $\mathcal{C} _0$;
...
\end{itemize}
Of course, one must show that composition is well-defined, but we are unfortunately out of time to show this. However, this does indeed follow, giving a well-defined notion of a homotopy category for a quasi-category $\mathcal{C} $. We have thus shown that $\mathcal{C} $ is a good model for an $\infty $-category (at least, up to the fairly informal standards of this paper).
