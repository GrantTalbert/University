...
\begin{construction}\label{7.1}
	...
	\cdots\ar[" F(d_{n+2} ) ", r]  & C_{n+1} \ar[" F(d_{n+1} ) ", r]  & C_n\ar[" F(d_n) ", r]  & C_{n-1} \ar[" F(d_{n-1} ) ", r]  & \cdots
	\end{tikzcd}\]
	...
	\end{tikzcd}\]
	Thus, $\Ch(F)(f_{\bullet} )$ is a morphism of chain complexes $\Ch(F)(C_{\bullet} )\to\Ch(F)(D_{\bullet} )$.
\end{construction}

With \cref{7.1} out of the way, we would like to know if additive functors preserve quasi-isomorphisms. Unfortunately, they do not in general. However, there is an important class of functions that \emph{does} preserve quasi-isomorphisms.
