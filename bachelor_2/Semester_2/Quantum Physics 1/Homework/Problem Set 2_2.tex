...
\begin{enumerate}
	...
	\item First we find the norm of the given vector.
		\[
			...
			1 \\
			i
			\end{pmatrix} \eqqcolon \frac{1}{\sqrt{2} } \begin{pmatrix}
			10+5i \\
			3+2i
			\end{pmatrix}\vec{v} 
		.\]
		\[
			\left\| \vec{v}  \right\| = \frac{1}{\sqrt{2} } \sqrt{\left| 10+5i \right|^2+\left| 10+5i \right|^2} =\frac{1}{\sqrt{2} } \sqrt{125+15} =\frac{\sqrt{140} }{\sqrt{2} } =\sqrt{70} 
		.\]
	It follows that
	\[
		\ket{\psi}  = \frac{1}{\sqrt{70} } \left( 3\ket{+}_x+5i\ket{-}_x+7\ket{+}_y \right) 
	.\]
	\item First, we compute the probabilities after the Z Stern-Gerlach machine.
		\begin{align*}
			\mathcal{P}_+ &=  \left| \bra{+}S_z\ket{\psi } \right| ^2\\
				      &= \left| \begin{pmatrix}
			1 & 0
			\end{pmatrix}\begin{pmatrix}
			1 &  \\
			 & -1
			\end{pmatrix}\left( \frac{1}{\sqrt{70} } \left( 3\ket{+}_x+5i\ket{-}_x+7\ket{+}_y  \right)\right)  \right|^2 \\
				      &=\left| \begin{pmatrix}
				      1 & 0
				      \end{pmatrix}\begin{pmatrix}
				      1 &  \\
				       & -1
				      \end{pmatrix}\frac{1}{\sqrt{140} } \begin{pmatrix}
				      10+5i \\
				      3+2i
				      \end{pmatrix} \right|^2\\
				      &=\left| \frac{1}{\sqrt{140} } \begin{pmatrix}
				      1 & 0
				      \end{pmatrix}\begin{pmatrix}
				      10+5i \\
				      -3-2i
				      \end{pmatrix} \right|^2\\
				      &=\frac{1}{140} \left| 10+5i \right|^2\\
				      &=\frac{125}{140}\\
				      &=\frac{25}{28} \\
			\mathcal{P}_- &= \left| \bra{-} S_z \ket{\psi }  \right|^2\\
				      &=\frac{1}{140} \left| 3+2i \right|^2\\
				      &=\frac{15}{140} \\
				      &=\frac{3}{28} 
		.\end{align*}
		After the first measurement, since we are measuring every time the particle goes through a machine, we will necessarily be in a basis state, so the distribution afterwards will be even. We simply divide our output by the number of possibe outcomes to obtain each probability.
		\[\begin{tikzcd}[row sep = 2.5em, column sep = 4em]
			&&&&25/112\\
			&&&&25/112\\
			&&&S_y\arrow[ru, "\ket{-}_y"']\arrow[ruu, "\ket{+}_y"]&25/112\\
			&&S_x\arrow[ru, "\ket{+}_x"]\arrow[r, "\ket{-}_x"']&S_y\arrow[ru, "\ket{+}_x"]\arrow[r, "\ket{-}_x"']&25/112\\
			\ket{\psi }\arrow[r]&S_z\arrow[ru, "\ket{+}"]\arrow[dr, "\ket{-} "'] \\
					    &&S_x\arrow[dr, "\ket{-}_x"']\arrow[r, "\ket{+}_x"]&S_y\arrow[dr, "\ket{-}_y"']\arrow[r, "\ket{+}_y"]&3/112\\
				   &&&S_y\arrow[ddr, "\ket{-}_y"']\arrow[dr, "\ket{+}_y"]&3/112\\
				   &&&&3/112\\
				   &&&&3/112
		\end{tikzcd}\]
\end{enumerate}
