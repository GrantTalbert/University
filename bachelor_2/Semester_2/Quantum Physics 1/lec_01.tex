...
\begin{definition}[Kronecker Delta]\label{dfn:1.1}
	The Kronecker Delta is denoted $\delta_{ij}$, and is defined as
	\[
		\delta_{ij}=
		\begin{cases}
			1,& i=j\\
	...
	.\]
\end{definition}
We can interpret $\delta_{ij}$ as the identity matrix $\mathbbm{1} $, since it has 1s only when the indices $i=j$. Let $\mathbf{a}$ be a vector. Then $\delta_{ij}a_j=a_i$ (in Einstein notation). 
