...
.\]
This contradicts our assumption that $\phi (a+bi)\neq 0$, so we have that $\phi (1)\neq 0$. Suppose that $\phi (1)=r$ for some $r\in\mathbb{Q} $. Note that $4\in\mathbb{Q} $. It follows that
\[
...
.\]
Therefore, $r^2=r$, and since $r\in\mathbb{Q} \setminus \left\{ 0 \right\} $, we have that $r=1$. Therefore, $\phi (1)=1$. It follows that $\phi (r)=r$ for all $r\in\mathbb{Z} $ by the same proof I gave in the previous homework, and thus $\phi (-1)=-1$.

Now consider $\phi (i)$. Note that $i^2=-1$, so
\[
	-1=\phi (-1)=\phi (i^2)=\phi (i)^2\implies \phi (i)=\pm\sqrt{-1} =\pm i
.\]
However, $\pm i\notin \mathbb{Q} $, which is a contradiction. Since $\phi (-1)=-1$ is guarenteed since $\phi (1)\neq 0$, this contradicts our original assumption that $\phi \neq 0$, and thus $\phi $ is the 0 homomorphism.
\section*{Problem 2}
...
It follows that $xy+yx=0$, and thus $xy=-yx$ for all $x,y\in R$. Note once again that $\operatorname{char} R=2$. It follows that for any $r\in R$,
\[
...
\[
	xy=-yx \implies xy=yx
.\]
Therefore, $R$ is commutative. This contradicts the fact that $R$ is noncommutative, and thus $\phi $ cannot be a homomorphism if $R$ is noncommutative.
\end{document}
