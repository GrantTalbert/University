...
\end{tikzcd}\]
In some cases the operation is left-multiplication, and in others the operation is right-multiplication. The important thing to note is that if there exists some $i\in \left\{ 1, \ldots , 4 \right\} $ with $M_i\in I$, then it follows that $M_i\in I$ for all $i$, because $I$ is an ideal and thus has the absorbtion property. I will now show the ideal contains at least one of these.
...
\[
	A=kM_i
\]
for some $i$. Since $I$ is an ideal and $\frac{1}{k} 1_R\in R$ for $1_R$ the identity matrix, we have $\frac{1}{k} 1_R A=\frac{k}{k} 1_RM_i=M_i$ is an element of the ideal.
...
\begin{itemize}
	\item \textbf{Case 1:} $a,b\neq 0$, and $c,d=0$. That is,
		...
		\[
			AM_4=\left( aM_1+bM_2 \right) =aM_1M_4+bM_2M_4=bM_2
		.\]
		Here, I use some facts about vector spaces, such as $a0=0$ for all scalars $a$. Since $I$ is an ideal and $a\in I$, we have $AM_4=bM_2\in I$. By the same logic, we also have
		\[
