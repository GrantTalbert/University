...
.\]
Due to this, $\phi $ can be viewed as a $\mathbb{Z} $-module homomorphism.
\section*{Problem 3}
...
.\]
Clearly, we must have $2rs\sqrt{2} =0$,\footnote{I don't feel like proving that $a+b\sqrt{2} =c+d\sqrt{2} $ iff $a=c$ and $b=d$. This can be shown by proving $\left\{ 1,\sqrt{2}  \right\} $ is a basis for $R$ when viewed as a $\mathbb{Z} $-module, then applying the fact that basis representations are unique but that's like so much work for something so obvious} and since $\mathbb{Z} $ is a domain, we must have either $r=0$ or $s=0$. If $s=0$, then $r^2=2$, and $r=\sqrt{2} \notin \mathbb{Z} $, a contradiction. Therefore, $r=0$. It follows that $2s^2=2$, and thus $s=\pm1$. Therefore,
\[
