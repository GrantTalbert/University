...
\[\begin{tikzcd}[row sep = 2.5em, column sep = 2.5em]
	A\arrow[r, "f"]\arrow[dr, "\pi "]&X\\
					 &f(A)\arrow[u]
\end{tikzcd}\]
We can define the relation $\sim $ on $X$ such that $x\sim y$ if and only if $y=f(x)$. This is trivially an equivalence relation, so we then have a surjection $X \to X/\tildesim$ given by the canonical projection, and thus any function $f : A \to X$ gives rise to a very natural quotient topology.
