...
\begin{document}
\makeproblem
% remark - show function in 1 is well-def
\section*{Problem 1}
\begin{proof}
	...
	Suppose $\lambda_a(b)=\lambda_a(c)$, implying that $ab=ac$. It follows that $a(b-c)=0$. Since $R$ has no zero divisors and $a\neq 0$, we have $b-c=0$. It follows that $b=c$, and  $\lambda_a $ is injective. Therefore, for all $a\in R^*$, there exists $1\in R$ such that $a1=1a=a$. Since there are no zero divisors and the ring is commutative, the cancellation law holds, so for any $b\in R$, it follows that
	\[
		a1b=ab\implies 1b=b
	.\]
	Therefore, $1$ is a unity in $R$.
\end{proof}
\section*{Problem 2}
